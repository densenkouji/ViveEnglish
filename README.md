...
| 要件 | 実装 |
|---|---|
| 長文読解サポート | 読解画面で入力文またはAI生成文を、段落単位でAI解析し、文分割・5文型・スラッシュ読み・句/節/熟語・ディスコースマーカーを色分け |
...
| `VIVE_READING_DEBUG` | `0` | `1`で、簡易解析に落ちた段落とその理由を `data/reading_debug.log` に記録（「一部の段落はAI解析が崩れた」原因の切り分け用） |
...
長文読解の文構造解析（段落内の文分割、主語・動詞、5文型、スラッシュ読み、句/節/熟語、段落役割など）は、段落ごとに厳密なJSONを返す高度なタスクです。
...
- 条件を満たさない場合は、ルールベースの**簡易解析**を目立つ警告付きで表示し、画面に高機能LLMの利用を促す案内を出します（他機能には影響しません）。
- モデル名に規模表記（`-7b` など）が無い高性能ローカルモデルを使う場合は、`VIVE_READING_FORCE_AI=1` でAI解析を強制有効化できます。
- 推論（reasoning）系モデル（o-series / gpt-5 系など）では、内部推論にトークンを使い切って本文が空で返ることがあります。ViveEnglish は空応答を検知するとトークン上限を自動的に引き上げて再試行します。それでも一部の段落が簡易解析になる場合は、`VIVE_READING_DEBUG=1` で `data/reading_debug.log` を確認してください（`finish_reason=length` なら上限到達が原因）。
...
トップナビの **読解** では、任意の英文を貼り付けるかAIで長文を生成して、段落ごとの文分割、文構造・5文型・スラッシュ読み・句/節/熟語・重要な接続語や指示語を色分けで確認できます。
